...
export interface DomainModel_5 {
  id: string;
  name: string;
  tier: number;
...
export class DomainServiceModule_5 {
  private repository: Map<string, DomainModel_5> = new Map();

  public register(name: string, tier: number = 1): DomainModel_5 {
    const id = 'dom-5-' + Math.random().toString(36).substring(2, 9);
    const item: DomainModel_5 = {
      id,
      name,
      tier,
      activeStatus: true,
  ...
  public getById(id: string): DomainModel_5 | undefined {
    return this.repository.get(id);
  }

  public listAll(): DomainModel_5[] {
