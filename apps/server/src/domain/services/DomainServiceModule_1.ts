...
export interface DomainModel_1 {
  id: string;
  name: string;
  tier: number;
...
export class DomainServiceModule_1 {
  private repository: Map<string, DomainModel_1> = new Map();

  public register(name: string, tier: number = 1): DomainModel_1 {
    const id = 'dom-1-' + Math.random().toString(36).substring(2, 9);
    const item: DomainModel_1 = {
      id,
      name,
      tier,
      activeStatus: true,
  ...
  public getById(id: string): DomainModel_1 | undefined {
    return this.repository.get(id);
  }

  public listAll(): DomainModel_1[] {
