...
export interface DomainModel_9 {
  id: string;
  name: string;
  tier: number;
...
export class DomainServiceModule_9 {
  private repository: Map<string, DomainModel_9> = new Map();

  public register(name: string, tier: number = 1): DomainModel_9 {
    const id = 'dom-9-' + Math.random().toString(36).substring(2, 9);
    const item: DomainModel_9 = {
      id,
      name,
      tier,
      activeStatus: true,
  ...
  public getById(id: string): DomainModel_9 | undefined {
    return this.repository.get(id);
  }

  public listAll(): DomainModel_9[] {
