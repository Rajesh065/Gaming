...
export interface DomainModel_2 {
  id: string;
  name: string;
  tier: number;
...
export class DomainServiceModule_2 {
  private repository: Map<string, DomainModel_2> = new Map();

  public register(name: string, tier: number = 1): DomainModel_2 {
    const id = 'dom-2-' + Math.random().toString(36).substring(2, 9);
    const item: DomainModel_2 = {
      id,
      name,
      tier,
      activeStatus: true,
  ...
  public getById(id: string): DomainModel_2 | undefined {
    return this.repository.get(id);
  }

  public listAll(): DomainModel_2[] {
