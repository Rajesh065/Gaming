...
export interface DomainModel_8 {
  id: string;
  name: string;
  tier: number;
...
export class DomainServiceModule_8 {
  private repository: Map<string, DomainModel_8> = new Map();

  public register(name: string, tier: number = 1): DomainModel_8 {
    const id = 'dom-8-' + Math.random().toString(36).substring(2, 9);
    const item: DomainModel_8 = {
      id,
      name,
      tier,
      activeStatus: true,
  ...
  public getById(id: string): DomainModel_8 | undefined {
    return this.repository.get(id);
  }

  public listAll(): DomainModel_8[] {
