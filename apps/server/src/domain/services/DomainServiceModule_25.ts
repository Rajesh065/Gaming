...
export interface DomainModel_25 {
  id: string;
  name: string;
  tier: number;
...
export class DomainServiceModule_25 {
  private repository: Map<string, DomainModel_25> = new Map();

  public register(name: string, tier: number = 1): DomainModel_25 {
    const id = 'dom-25-' + Math.random().toString(36).substring(2, 9);
    const item: DomainModel_25 = {
      id,
      name,
      tier,
      activeStatus: true,
  ...
  public getById(id: string): DomainModel_25 | undefined {
    return this.repository.get(id);
  }

  public listAll(): DomainModel_25[] {
