...
export interface DomainModel_20 {
  id: string;
  name: string;
  tier: number;
...
export class DomainServiceModule_20 {
  private repository: Map<string, DomainModel_20> = new Map();

  public register(name: string, tier: number = 1): DomainModel_20 {
    const id = 'dom-20-' + Math.random().toString(36).substring(2, 9);
    const item: DomainModel_20 = {
      id,
      name,
      tier,
      activeStatus: true,
  ...
  public getById(id: string): DomainModel_20 | undefined {
    return this.repository.get(id);
  }

  public listAll(): DomainModel_20[] {
