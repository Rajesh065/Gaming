...
export enum NodeStatus_35 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_35 {
  public name: string;
  public children: BehaviorNode_35[] = [];
  ...
  public evaluate(context: AIContext_35, deltaTime: number): NodeStatus_35 {
    if (context.health <= 0) return NodeStatus_35.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_35, dt: number): NodeStatus_35 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_35.RUNNING;
  }

  private executeCombatLogic(context: AIContext_35, dt: number): NodeStatus_35 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_35.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_35, dt: number): NodeStatus_35 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_35.RUNNING;
  }

  private executePatrolLogic(context: AIContext_35, dt: number): NodeStatus_35 {
