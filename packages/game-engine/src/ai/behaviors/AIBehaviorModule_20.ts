...
export enum NodeStatus_20 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_20 {
  public name: string;
  public children: BehaviorNode_20[] = [];
  ...
  public evaluate(context: AIContext_20, deltaTime: number): NodeStatus_20 {
    if (context.health <= 0) return NodeStatus_20.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_20, dt: number): NodeStatus_20 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_20.RUNNING;
  }

  private executeCombatLogic(context: AIContext_20, dt: number): NodeStatus_20 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_20.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_20, dt: number): NodeStatus_20 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_20.RUNNING;
  }

  private executePatrolLogic(context: AIContext_20, dt: number): NodeStatus_20 {
