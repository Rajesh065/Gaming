...
export enum NodeStatus_29 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_29 {
  public name: string;
  public children: BehaviorNode_29[] = [];
  ...
  public evaluate(context: AIContext_29, deltaTime: number): NodeStatus_29 {
    if (context.health <= 0) return NodeStatus_29.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_29, dt: number): NodeStatus_29 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_29.RUNNING;
  }

  private executeCombatLogic(context: AIContext_29, dt: number): NodeStatus_29 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_29.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_29, dt: number): NodeStatus_29 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_29.RUNNING;
  }

  private executePatrolLogic(context: AIContext_29, dt: number): NodeStatus_29 {
