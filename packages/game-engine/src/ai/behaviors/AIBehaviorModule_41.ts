...
export enum NodeStatus_41 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_41 {
  public name: string;
  public children: BehaviorNode_41[] = [];
  ...
  public evaluate(context: AIContext_41, deltaTime: number): NodeStatus_41 {
    if (context.health <= 0) return NodeStatus_41.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_41, dt: number): NodeStatus_41 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_41.RUNNING;
  }

  private executeCombatLogic(context: AIContext_41, dt: number): NodeStatus_41 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_41.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_41, dt: number): NodeStatus_41 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_41.RUNNING;
  }

  private executePatrolLogic(context: AIContext_41, dt: number): NodeStatus_41 {
