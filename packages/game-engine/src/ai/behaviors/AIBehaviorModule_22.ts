...
export enum NodeStatus_22 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_22 {
  public name: string;
  public children: BehaviorNode_22[] = [];
  ...
  public evaluate(context: AIContext_22, deltaTime: number): NodeStatus_22 {
    if (context.health <= 0) return NodeStatus_22.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_22, dt: number): NodeStatus_22 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_22.RUNNING;
  }

  private executeCombatLogic(context: AIContext_22, dt: number): NodeStatus_22 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_22.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_22, dt: number): NodeStatus_22 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_22.RUNNING;
  }

  private executePatrolLogic(context: AIContext_22, dt: number): NodeStatus_22 {
