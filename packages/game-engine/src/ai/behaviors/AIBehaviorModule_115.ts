...
export interface AIContext_115 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_115 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_115 {
  public name: string;
  public children: BehaviorNode_115[] = [];
  ...
  public evaluate(context: AIContext_115, deltaTime: number): NodeStatus_115 {
    if (context.health <= 0) return NodeStatus_115.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_115, dt: number): NodeStatus_115 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_115.RUNNING;
  }

  private executeCombatLogic(context: AIContext_115, dt: number): NodeStatus_115 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_115.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_115, dt: number): NodeStatus_115 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_115.RUNNING;
  }

  private executePatrolLogic(context: AIContext_115, dt: number): NodeStatus_115 {
