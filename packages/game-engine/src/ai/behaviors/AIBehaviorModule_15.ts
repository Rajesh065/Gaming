...
export enum NodeStatus_15 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_15 {
  public name: string;
  public children: BehaviorNode_15[] = [];
  ...
  public evaluate(context: AIContext_15, deltaTime: number): NodeStatus_15 {
    if (context.health <= 0) return NodeStatus_15.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_15, dt: number): NodeStatus_15 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_15.RUNNING;
  }

  private executeCombatLogic(context: AIContext_15, dt: number): NodeStatus_15 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_15.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_15, dt: number): NodeStatus_15 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_15.RUNNING;
  }

  private executePatrolLogic(context: AIContext_15, dt: number): NodeStatus_15 {
