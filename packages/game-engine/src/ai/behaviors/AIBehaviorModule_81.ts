...
export enum NodeStatus_81 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_81 {
  public name: string;
  public children: BehaviorNode_81[] = [];
  ...
  public evaluate(context: AIContext_81, deltaTime: number): NodeStatus_81 {
    if (context.health <= 0) return NodeStatus_81.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_81, dt: number): NodeStatus_81 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_81.RUNNING;
  }

  private executeCombatLogic(context: AIContext_81, dt: number): NodeStatus_81 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_81.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_81, dt: number): NodeStatus_81 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_81.RUNNING;
  }

  private executePatrolLogic(context: AIContext_81, dt: number): NodeStatus_81 {
