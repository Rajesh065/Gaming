...
export enum NodeStatus_12 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_12 {
  public name: string;
  public children: BehaviorNode_12[] = [];
  ...
  public evaluate(context: AIContext_12, deltaTime: number): NodeStatus_12 {
    if (context.health <= 0) return NodeStatus_12.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_12, dt: number): NodeStatus_12 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_12.RUNNING;
  }

  private executeCombatLogic(context: AIContext_12, dt: number): NodeStatus_12 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_12.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_12, dt: number): NodeStatus_12 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_12.RUNNING;
  }

  private executePatrolLogic(context: AIContext_12, dt: number): NodeStatus_12 {
