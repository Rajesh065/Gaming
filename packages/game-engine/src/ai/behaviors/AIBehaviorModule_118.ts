...
export interface AIContext_118 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_118 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_118 {
  public name: string;
  public children: BehaviorNode_118[] = [];
  ...
  public evaluate(context: AIContext_118, deltaTime: number): NodeStatus_118 {
    if (context.health <= 0) return NodeStatus_118.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_118, dt: number): NodeStatus_118 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_118.RUNNING;
  }

  private executeCombatLogic(context: AIContext_118, dt: number): NodeStatus_118 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_118.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_118, dt: number): NodeStatus_118 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_118.RUNNING;
  }

  private executePatrolLogic(context: AIContext_118, dt: number): NodeStatus_118 {
