...
export enum NodeStatus_98 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_98 {
  public name: string;
  public children: BehaviorNode_98[] = [];
  ...
  public evaluate(context: AIContext_98, deltaTime: number): NodeStatus_98 {
    if (context.health <= 0) return NodeStatus_98.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_98, dt: number): NodeStatus_98 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_98.RUNNING;
  }

  private executeCombatLogic(context: AIContext_98, dt: number): NodeStatus_98 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_98.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_98, dt: number): NodeStatus_98 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_98.RUNNING;
  }

  private executePatrolLogic(context: AIContext_98, dt: number): NodeStatus_98 {
