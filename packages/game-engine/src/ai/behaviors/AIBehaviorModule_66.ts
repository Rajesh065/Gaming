...
export enum NodeStatus_66 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_66 {
  public name: string;
  public children: BehaviorNode_66[] = [];
  ...
  public evaluate(context: AIContext_66, deltaTime: number): NodeStatus_66 {
    if (context.health <= 0) return NodeStatus_66.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_66, dt: number): NodeStatus_66 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_66.RUNNING;
  }

  private executeCombatLogic(context: AIContext_66, dt: number): NodeStatus_66 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_66.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_66, dt: number): NodeStatus_66 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_66.RUNNING;
  }

  private executePatrolLogic(context: AIContext_66, dt: number): NodeStatus_66 {
