...
export interface AIContext_111 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_111 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_111 {
  public name: string;
  public children: BehaviorNode_111[] = [];
  ...
  public evaluate(context: AIContext_111, deltaTime: number): NodeStatus_111 {
    if (context.health <= 0) return NodeStatus_111.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_111, dt: number): NodeStatus_111 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_111.RUNNING;
  }

  private executeCombatLogic(context: AIContext_111, dt: number): NodeStatus_111 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_111.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_111, dt: number): NodeStatus_111 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_111.RUNNING;
  }

  private executePatrolLogic(context: AIContext_111, dt: number): NodeStatus_111 {
