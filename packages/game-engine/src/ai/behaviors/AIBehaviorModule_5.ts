...
export enum NodeStatus_5 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_5 {
  public name: string;
  public children: BehaviorNode_5[] = [];
  ...
  public evaluate(context: AIContext_5, deltaTime: number): NodeStatus_5 {
    if (context.health <= 0) return NodeStatus_5.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_5, dt: number): NodeStatus_5 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_5.RUNNING;
  }

  private executeCombatLogic(context: AIContext_5, dt: number): NodeStatus_5 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_5.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_5, dt: number): NodeStatus_5 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_5.RUNNING;
  }

  private executePatrolLogic(context: AIContext_5, dt: number): NodeStatus_5 {
