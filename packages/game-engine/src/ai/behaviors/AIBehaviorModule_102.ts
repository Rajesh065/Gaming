...
export interface AIContext_102 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_102 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_102 {
  public name: string;
  public children: BehaviorNode_102[] = [];
  ...
  public evaluate(context: AIContext_102, deltaTime: number): NodeStatus_102 {
    if (context.health <= 0) return NodeStatus_102.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_102, dt: number): NodeStatus_102 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_102.RUNNING;
  }

  private executeCombatLogic(context: AIContext_102, dt: number): NodeStatus_102 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_102.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_102, dt: number): NodeStatus_102 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_102.RUNNING;
  }

  private executePatrolLogic(context: AIContext_102, dt: number): NodeStatus_102 {
