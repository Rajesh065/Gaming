...
export interface AIContext_100 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_100 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_100 {
  public name: string;
  public children: BehaviorNode_100[] = [];
  ...
  public evaluate(context: AIContext_100, deltaTime: number): NodeStatus_100 {
    if (context.health <= 0) return NodeStatus_100.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_100, dt: number): NodeStatus_100 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_100.RUNNING;
  }

  private executeCombatLogic(context: AIContext_100, dt: number): NodeStatus_100 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_100.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_100, dt: number): NodeStatus_100 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_100.RUNNING;
  }

  private executePatrolLogic(context: AIContext_100, dt: number): NodeStatus_100 {
