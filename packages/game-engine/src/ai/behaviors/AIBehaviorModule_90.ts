...
export enum NodeStatus_90 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_90 {
  public name: string;
  public children: BehaviorNode_90[] = [];
  ...
  public evaluate(context: AIContext_90, deltaTime: number): NodeStatus_90 {
    if (context.health <= 0) return NodeStatus_90.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_90, dt: number): NodeStatus_90 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_90.RUNNING;
  }

  private executeCombatLogic(context: AIContext_90, dt: number): NodeStatus_90 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_90.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_90, dt: number): NodeStatus_90 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_90.RUNNING;
  }

  private executePatrolLogic(context: AIContext_90, dt: number): NodeStatus_90 {
