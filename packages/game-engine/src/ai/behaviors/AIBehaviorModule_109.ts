...
export interface AIContext_109 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_109 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_109 {
  public name: string;
  public children: BehaviorNode_109[] = [];
  ...
  public evaluate(context: AIContext_109, deltaTime: number): NodeStatus_109 {
    if (context.health <= 0) return NodeStatus_109.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_109, dt: number): NodeStatus_109 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_109.RUNNING;
  }

  private executeCombatLogic(context: AIContext_109, dt: number): NodeStatus_109 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_109.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_109, dt: number): NodeStatus_109 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_109.RUNNING;
  }

  private executePatrolLogic(context: AIContext_109, dt: number): NodeStatus_109 {
