...
export enum NodeStatus_40 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_40 {
  public name: string;
  public children: BehaviorNode_40[] = [];
  ...
  public evaluate(context: AIContext_40, deltaTime: number): NodeStatus_40 {
    if (context.health <= 0) return NodeStatus_40.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_40, dt: number): NodeStatus_40 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_40.RUNNING;
  }

  private executeCombatLogic(context: AIContext_40, dt: number): NodeStatus_40 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_40.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_40, dt: number): NodeStatus_40 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_40.RUNNING;
  }

  private executePatrolLogic(context: AIContext_40, dt: number): NodeStatus_40 {
