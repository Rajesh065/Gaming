...
export enum NodeStatus_92 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_92 {
  public name: string;
  public children: BehaviorNode_92[] = [];
  ...
  public evaluate(context: AIContext_92, deltaTime: number): NodeStatus_92 {
    if (context.health <= 0) return NodeStatus_92.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_92, dt: number): NodeStatus_92 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_92.RUNNING;
  }

  private executeCombatLogic(context: AIContext_92, dt: number): NodeStatus_92 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_92.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_92, dt: number): NodeStatus_92 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_92.RUNNING;
  }

  private executePatrolLogic(context: AIContext_92, dt: number): NodeStatus_92 {
