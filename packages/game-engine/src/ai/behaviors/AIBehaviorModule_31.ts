...
export enum NodeStatus_31 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_31 {
  public name: string;
  public children: BehaviorNode_31[] = [];
  ...
  public evaluate(context: AIContext_31, deltaTime: number): NodeStatus_31 {
    if (context.health <= 0) return NodeStatus_31.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_31, dt: number): NodeStatus_31 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_31.RUNNING;
  }

  private executeCombatLogic(context: AIContext_31, dt: number): NodeStatus_31 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_31.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_31, dt: number): NodeStatus_31 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_31.RUNNING;
  }

  private executePatrolLogic(context: AIContext_31, dt: number): NodeStatus_31 {
