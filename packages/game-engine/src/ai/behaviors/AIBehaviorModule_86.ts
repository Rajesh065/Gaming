...
export enum NodeStatus_86 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_86 {
  public name: string;
  public children: BehaviorNode_86[] = [];
  ...
  public evaluate(context: AIContext_86, deltaTime: number): NodeStatus_86 {
    if (context.health <= 0) return NodeStatus_86.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_86, dt: number): NodeStatus_86 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_86.RUNNING;
  }

  private executeCombatLogic(context: AIContext_86, dt: number): NodeStatus_86 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_86.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_86, dt: number): NodeStatus_86 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_86.RUNNING;
  }

  private executePatrolLogic(context: AIContext_86, dt: number): NodeStatus_86 {
