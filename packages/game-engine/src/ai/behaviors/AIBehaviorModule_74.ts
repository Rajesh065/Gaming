...
export enum NodeStatus_74 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_74 {
  public name: string;
  public children: BehaviorNode_74[] = [];
  ...
  public evaluate(context: AIContext_74, deltaTime: number): NodeStatus_74 {
    if (context.health <= 0) return NodeStatus_74.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_74, dt: number): NodeStatus_74 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_74.RUNNING;
  }

  private executeCombatLogic(context: AIContext_74, dt: number): NodeStatus_74 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_74.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_74, dt: number): NodeStatus_74 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_74.RUNNING;
  }

  private executePatrolLogic(context: AIContext_74, dt: number): NodeStatus_74 {
