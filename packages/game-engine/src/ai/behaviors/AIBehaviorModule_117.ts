...
export interface AIContext_117 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_117 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_117 {
  public name: string;
  public children: BehaviorNode_117[] = [];
  ...
  public evaluate(context: AIContext_117, deltaTime: number): NodeStatus_117 {
    if (context.health <= 0) return NodeStatus_117.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_117, dt: number): NodeStatus_117 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_117.RUNNING;
  }

  private executeCombatLogic(context: AIContext_117, dt: number): NodeStatus_117 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_117.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_117, dt: number): NodeStatus_117 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_117.RUNNING;
  }

  private executePatrolLogic(context: AIContext_117, dt: number): NodeStatus_117 {
