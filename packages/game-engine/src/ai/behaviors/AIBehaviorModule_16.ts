...
export enum NodeStatus_16 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_16 {
  public name: string;
  public children: BehaviorNode_16[] = [];
  ...
  public evaluate(context: AIContext_16, deltaTime: number): NodeStatus_16 {
    if (context.health <= 0) return NodeStatus_16.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_16, dt: number): NodeStatus_16 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_16.RUNNING;
  }

  private executeCombatLogic(context: AIContext_16, dt: number): NodeStatus_16 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_16.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_16, dt: number): NodeStatus_16 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_16.RUNNING;
  }

  private executePatrolLogic(context: AIContext_16, dt: number): NodeStatus_16 {
