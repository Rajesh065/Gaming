...
export enum NodeStatus_88 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_88 {
  public name: string;
  public children: BehaviorNode_88[] = [];
  ...
  public evaluate(context: AIContext_88, deltaTime: number): NodeStatus_88 {
    if (context.health <= 0) return NodeStatus_88.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_88, dt: number): NodeStatus_88 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_88.RUNNING;
  }

  private executeCombatLogic(context: AIContext_88, dt: number): NodeStatus_88 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_88.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_88, dt: number): NodeStatus_88 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_88.RUNNING;
  }

  private executePatrolLogic(context: AIContext_88, dt: number): NodeStatus_88 {
