...
export enum NodeStatus_39 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_39 {
  public name: string;
  public children: BehaviorNode_39[] = [];
  ...
  public evaluate(context: AIContext_39, deltaTime: number): NodeStatus_39 {
    if (context.health <= 0) return NodeStatus_39.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_39, dt: number): NodeStatus_39 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_39.RUNNING;
  }

  private executeCombatLogic(context: AIContext_39, dt: number): NodeStatus_39 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_39.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_39, dt: number): NodeStatus_39 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_39.RUNNING;
  }

  private executePatrolLogic(context: AIContext_39, dt: number): NodeStatus_39 {
