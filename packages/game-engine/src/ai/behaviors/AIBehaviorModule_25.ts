...
export enum NodeStatus_25 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_25 {
  public name: string;
  public children: BehaviorNode_25[] = [];
  ...
  public evaluate(context: AIContext_25, deltaTime: number): NodeStatus_25 {
    if (context.health <= 0) return NodeStatus_25.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_25, dt: number): NodeStatus_25 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_25.RUNNING;
  }

  private executeCombatLogic(context: AIContext_25, dt: number): NodeStatus_25 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_25.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_25, dt: number): NodeStatus_25 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_25.RUNNING;
  }

  private executePatrolLogic(context: AIContext_25, dt: number): NodeStatus_25 {
