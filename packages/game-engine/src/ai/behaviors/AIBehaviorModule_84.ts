...
export enum NodeStatus_84 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_84 {
  public name: string;
  public children: BehaviorNode_84[] = [];
  ...
  public evaluate(context: AIContext_84, deltaTime: number): NodeStatus_84 {
    if (context.health <= 0) return NodeStatus_84.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_84, dt: number): NodeStatus_84 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_84.RUNNING;
  }

  private executeCombatLogic(context: AIContext_84, dt: number): NodeStatus_84 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_84.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_84, dt: number): NodeStatus_84 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_84.RUNNING;
  }

  private executePatrolLogic(context: AIContext_84, dt: number): NodeStatus_84 {
