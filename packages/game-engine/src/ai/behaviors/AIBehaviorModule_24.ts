...
export enum NodeStatus_24 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_24 {
  public name: string;
  public children: BehaviorNode_24[] = [];
  ...
  public evaluate(context: AIContext_24, deltaTime: number): NodeStatus_24 {
    if (context.health <= 0) return NodeStatus_24.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_24, dt: number): NodeStatus_24 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_24.RUNNING;
  }

  private executeCombatLogic(context: AIContext_24, dt: number): NodeStatus_24 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_24.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_24, dt: number): NodeStatus_24 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_24.RUNNING;
  }

  private executePatrolLogic(context: AIContext_24, dt: number): NodeStatus_24 {
