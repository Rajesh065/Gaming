...
export enum NodeStatus_70 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_70 {
  public name: string;
  public children: BehaviorNode_70[] = [];
  ...
  public evaluate(context: AIContext_70, deltaTime: number): NodeStatus_70 {
    if (context.health <= 0) return NodeStatus_70.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_70, dt: number): NodeStatus_70 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_70.RUNNING;
  }

  private executeCombatLogic(context: AIContext_70, dt: number): NodeStatus_70 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_70.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_70, dt: number): NodeStatus_70 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_70.RUNNING;
  }

  private executePatrolLogic(context: AIContext_70, dt: number): NodeStatus_70 {
