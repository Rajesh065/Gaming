...
export enum NodeStatus_14 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_14 {
  public name: string;
  public children: BehaviorNode_14[] = [];
  ...
  public evaluate(context: AIContext_14, deltaTime: number): NodeStatus_14 {
    if (context.health <= 0) return NodeStatus_14.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_14, dt: number): NodeStatus_14 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_14.RUNNING;
  }

  private executeCombatLogic(context: AIContext_14, dt: number): NodeStatus_14 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_14.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_14, dt: number): NodeStatus_14 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_14.RUNNING;
  }

  private executePatrolLogic(context: AIContext_14, dt: number): NodeStatus_14 {
