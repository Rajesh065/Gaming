...
export enum NodeStatus_11 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_11 {
  public name: string;
  public children: BehaviorNode_11[] = [];
  ...
  public evaluate(context: AIContext_11, deltaTime: number): NodeStatus_11 {
    if (context.health <= 0) return NodeStatus_11.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_11, dt: number): NodeStatus_11 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_11.RUNNING;
  }

  private executeCombatLogic(context: AIContext_11, dt: number): NodeStatus_11 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_11.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_11, dt: number): NodeStatus_11 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_11.RUNNING;
  }

  private executePatrolLogic(context: AIContext_11, dt: number): NodeStatus_11 {
