...
export interface AIContext_119 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_119 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_119 {
  public name: string;
  public children: BehaviorNode_119[] = [];
  ...
  public evaluate(context: AIContext_119, deltaTime: number): NodeStatus_119 {
    if (context.health <= 0) return NodeStatus_119.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_119, dt: number): NodeStatus_119 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_119.RUNNING;
  }

  private executeCombatLogic(context: AIContext_119, dt: number): NodeStatus_119 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_119.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_119, dt: number): NodeStatus_119 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_119.RUNNING;
  }

  private executePatrolLogic(context: AIContext_119, dt: number): NodeStatus_119 {
