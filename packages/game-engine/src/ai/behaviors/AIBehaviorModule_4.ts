...
export enum NodeStatus_4 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_4 {
  public name: string;
  public children: BehaviorNode_4[] = [];
  ...
  public evaluate(context: AIContext_4, deltaTime: number): NodeStatus_4 {
    if (context.health <= 0) return NodeStatus_4.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_4, dt: number): NodeStatus_4 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_4.RUNNING;
  }

  private executeCombatLogic(context: AIContext_4, dt: number): NodeStatus_4 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_4.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_4, dt: number): NodeStatus_4 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_4.RUNNING;
  }

  private executePatrolLogic(context: AIContext_4, dt: number): NodeStatus_4 {
