...
export enum NodeStatus_79 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_79 {
  public name: string;
  public children: BehaviorNode_79[] = [];
  ...
  public evaluate(context: AIContext_79, deltaTime: number): NodeStatus_79 {
    if (context.health <= 0) return NodeStatus_79.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_79, dt: number): NodeStatus_79 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_79.RUNNING;
  }

  private executeCombatLogic(context: AIContext_79, dt: number): NodeStatus_79 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_79.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_79, dt: number): NodeStatus_79 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_79.RUNNING;
  }

  private executePatrolLogic(context: AIContext_79, dt: number): NodeStatus_79 {
