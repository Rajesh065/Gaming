...
export enum NodeStatus_17 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_17 {
  public name: string;
  public children: BehaviorNode_17[] = [];
  ...
  public evaluate(context: AIContext_17, deltaTime: number): NodeStatus_17 {
    if (context.health <= 0) return NodeStatus_17.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_17, dt: number): NodeStatus_17 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_17.RUNNING;
  }

  private executeCombatLogic(context: AIContext_17, dt: number): NodeStatus_17 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_17.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_17, dt: number): NodeStatus_17 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_17.RUNNING;
  }

  private executePatrolLogic(context: AIContext_17, dt: number): NodeStatus_17 {
