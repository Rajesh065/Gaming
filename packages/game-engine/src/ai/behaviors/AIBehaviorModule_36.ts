...
export enum NodeStatus_36 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_36 {
  public name: string;
  public children: BehaviorNode_36[] = [];
  ...
  public evaluate(context: AIContext_36, deltaTime: number): NodeStatus_36 {
    if (context.health <= 0) return NodeStatus_36.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_36, dt: number): NodeStatus_36 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_36.RUNNING;
  }

  private executeCombatLogic(context: AIContext_36, dt: number): NodeStatus_36 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_36.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_36, dt: number): NodeStatus_36 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_36.RUNNING;
  }

  private executePatrolLogic(context: AIContext_36, dt: number): NodeStatus_36 {
