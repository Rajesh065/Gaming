...
export enum NodeStatus_28 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_28 {
  public name: string;
  public children: BehaviorNode_28[] = [];
  ...
  public evaluate(context: AIContext_28, deltaTime: number): NodeStatus_28 {
    if (context.health <= 0) return NodeStatus_28.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_28, dt: number): NodeStatus_28 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_28.RUNNING;
  }

  private executeCombatLogic(context: AIContext_28, dt: number): NodeStatus_28 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_28.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_28, dt: number): NodeStatus_28 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_28.RUNNING;
  }

  private executePatrolLogic(context: AIContext_28, dt: number): NodeStatus_28 {
