...
export enum NodeStatus_61 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_61 {
  public name: string;
  public children: BehaviorNode_61[] = [];
  ...
  public evaluate(context: AIContext_61, deltaTime: number): NodeStatus_61 {
    if (context.health <= 0) return NodeStatus_61.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_61, dt: number): NodeStatus_61 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_61.RUNNING;
  }

  private executeCombatLogic(context: AIContext_61, dt: number): NodeStatus_61 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_61.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_61, dt: number): NodeStatus_61 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_61.RUNNING;
  }

  private executePatrolLogic(context: AIContext_61, dt: number): NodeStatus_61 {
