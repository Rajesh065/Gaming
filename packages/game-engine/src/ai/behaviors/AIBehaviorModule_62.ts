...
export enum NodeStatus_62 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_62 {
  public name: string;
  public children: BehaviorNode_62[] = [];
  ...
  public evaluate(context: AIContext_62, deltaTime: number): NodeStatus_62 {
    if (context.health <= 0) return NodeStatus_62.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_62, dt: number): NodeStatus_62 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_62.RUNNING;
  }

  private executeCombatLogic(context: AIContext_62, dt: number): NodeStatus_62 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_62.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_62, dt: number): NodeStatus_62 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_62.RUNNING;
  }

  private executePatrolLogic(context: AIContext_62, dt: number): NodeStatus_62 {
