...
export enum NodeStatus_89 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_89 {
  public name: string;
  public children: BehaviorNode_89[] = [];
  ...
  public evaluate(context: AIContext_89, deltaTime: number): NodeStatus_89 {
    if (context.health <= 0) return NodeStatus_89.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_89, dt: number): NodeStatus_89 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_89.RUNNING;
  }

  private executeCombatLogic(context: AIContext_89, dt: number): NodeStatus_89 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_89.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_89, dt: number): NodeStatus_89 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_89.RUNNING;
  }

  private executePatrolLogic(context: AIContext_89, dt: number): NodeStatus_89 {
