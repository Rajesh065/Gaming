...
export enum NodeStatus_18 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_18 {
  public name: string;
  public children: BehaviorNode_18[] = [];
  ...
  public evaluate(context: AIContext_18, deltaTime: number): NodeStatus_18 {
    if (context.health <= 0) return NodeStatus_18.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_18, dt: number): NodeStatus_18 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_18.RUNNING;
  }

  private executeCombatLogic(context: AIContext_18, dt: number): NodeStatus_18 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_18.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_18, dt: number): NodeStatus_18 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_18.RUNNING;
  }

  private executePatrolLogic(context: AIContext_18, dt: number): NodeStatus_18 {
