...
export enum NodeStatus_44 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_44 {
  public name: string;
  public children: BehaviorNode_44[] = [];
  ...
  public evaluate(context: AIContext_44, deltaTime: number): NodeStatus_44 {
    if (context.health <= 0) return NodeStatus_44.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_44, dt: number): NodeStatus_44 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_44.RUNNING;
  }

  private executeCombatLogic(context: AIContext_44, dt: number): NodeStatus_44 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_44.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_44, dt: number): NodeStatus_44 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_44.RUNNING;
  }

  private executePatrolLogic(context: AIContext_44, dt: number): NodeStatus_44 {
