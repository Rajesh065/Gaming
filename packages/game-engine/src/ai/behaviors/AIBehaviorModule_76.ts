...
export enum NodeStatus_76 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_76 {
  public name: string;
  public children: BehaviorNode_76[] = [];
  ...
  public evaluate(context: AIContext_76, deltaTime: number): NodeStatus_76 {
    if (context.health <= 0) return NodeStatus_76.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_76, dt: number): NodeStatus_76 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_76.RUNNING;
  }

  private executeCombatLogic(context: AIContext_76, dt: number): NodeStatus_76 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_76.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_76, dt: number): NodeStatus_76 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_76.RUNNING;
  }

  private executePatrolLogic(context: AIContext_76, dt: number): NodeStatus_76 {
