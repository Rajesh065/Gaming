...
export enum NodeStatus_97 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_97 {
  public name: string;
  public children: BehaviorNode_97[] = [];
  ...
  public evaluate(context: AIContext_97, deltaTime: number): NodeStatus_97 {
    if (context.health <= 0) return NodeStatus_97.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_97, dt: number): NodeStatus_97 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_97.RUNNING;
  }

  private executeCombatLogic(context: AIContext_97, dt: number): NodeStatus_97 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_97.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_97, dt: number): NodeStatus_97 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_97.RUNNING;
  }

  private executePatrolLogic(context: AIContext_97, dt: number): NodeStatus_97 {
