...
export enum NodeStatus_50 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_50 {
  public name: string;
  public children: BehaviorNode_50[] = [];
  ...
  public evaluate(context: AIContext_50, deltaTime: number): NodeStatus_50 {
    if (context.health <= 0) return NodeStatus_50.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_50, dt: number): NodeStatus_50 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_50.RUNNING;
  }

  private executeCombatLogic(context: AIContext_50, dt: number): NodeStatus_50 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_50.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_50, dt: number): NodeStatus_50 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_50.RUNNING;
  }

  private executePatrolLogic(context: AIContext_50, dt: number): NodeStatus_50 {
