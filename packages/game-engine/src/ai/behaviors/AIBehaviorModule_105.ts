...
export interface AIContext_105 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_105 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_105 {
  public name: string;
  public children: BehaviorNode_105[] = [];
  ...
  public evaluate(context: AIContext_105, deltaTime: number): NodeStatus_105 {
    if (context.health <= 0) return NodeStatus_105.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_105, dt: number): NodeStatus_105 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_105.RUNNING;
  }

  private executeCombatLogic(context: AIContext_105, dt: number): NodeStatus_105 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_105.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_105, dt: number): NodeStatus_105 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_105.RUNNING;
  }

  private executePatrolLogic(context: AIContext_105, dt: number): NodeStatus_105 {
