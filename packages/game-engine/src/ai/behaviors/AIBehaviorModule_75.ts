...
export enum NodeStatus_75 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_75 {
  public name: string;
  public children: BehaviorNode_75[] = [];
  ...
  public evaluate(context: AIContext_75, deltaTime: number): NodeStatus_75 {
    if (context.health <= 0) return NodeStatus_75.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_75, dt: number): NodeStatus_75 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_75.RUNNING;
  }

  private executeCombatLogic(context: AIContext_75, dt: number): NodeStatus_75 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_75.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_75, dt: number): NodeStatus_75 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_75.RUNNING;
  }

  private executePatrolLogic(context: AIContext_75, dt: number): NodeStatus_75 {
