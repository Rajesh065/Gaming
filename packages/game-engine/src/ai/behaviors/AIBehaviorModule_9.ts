...
export enum NodeStatus_9 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_9 {
  public name: string;
  public children: BehaviorNode_9[] = [];
  ...
  public evaluate(context: AIContext_9, deltaTime: number): NodeStatus_9 {
    if (context.health <= 0) return NodeStatus_9.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_9, dt: number): NodeStatus_9 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_9.RUNNING;
  }

  private executeCombatLogic(context: AIContext_9, dt: number): NodeStatus_9 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_9.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_9, dt: number): NodeStatus_9 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_9.RUNNING;
  }

  private executePatrolLogic(context: AIContext_9, dt: number): NodeStatus_9 {
