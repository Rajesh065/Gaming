...
export enum NodeStatus_51 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_51 {
  public name: string;
  public children: BehaviorNode_51[] = [];
  ...
  public evaluate(context: AIContext_51, deltaTime: number): NodeStatus_51 {
    if (context.health <= 0) return NodeStatus_51.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_51, dt: number): NodeStatus_51 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_51.RUNNING;
  }

  private executeCombatLogic(context: AIContext_51, dt: number): NodeStatus_51 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_51.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_51, dt: number): NodeStatus_51 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_51.RUNNING;
  }

  private executePatrolLogic(context: AIContext_51, dt: number): NodeStatus_51 {
