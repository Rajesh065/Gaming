...
export interface AIContext_113 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_113 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_113 {
  public name: string;
  public children: BehaviorNode_113[] = [];
  ...
  public evaluate(context: AIContext_113, deltaTime: number): NodeStatus_113 {
    if (context.health <= 0) return NodeStatus_113.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_113, dt: number): NodeStatus_113 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_113.RUNNING;
  }

  private executeCombatLogic(context: AIContext_113, dt: number): NodeStatus_113 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_113.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_113, dt: number): NodeStatus_113 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_113.RUNNING;
  }

  private executePatrolLogic(context: AIContext_113, dt: number): NodeStatus_113 {
