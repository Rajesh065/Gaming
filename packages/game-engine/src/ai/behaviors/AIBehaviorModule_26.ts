...
export enum NodeStatus_26 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_26 {
  public name: string;
  public children: BehaviorNode_26[] = [];
  ...
  public evaluate(context: AIContext_26, deltaTime: number): NodeStatus_26 {
    if (context.health <= 0) return NodeStatus_26.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_26, dt: number): NodeStatus_26 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_26.RUNNING;
  }

  private executeCombatLogic(context: AIContext_26, dt: number): NodeStatus_26 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_26.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_26, dt: number): NodeStatus_26 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_26.RUNNING;
  }

  private executePatrolLogic(context: AIContext_26, dt: number): NodeStatus_26 {
