...
export enum NodeStatus_21 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_21 {
  public name: string;
  public children: BehaviorNode_21[] = [];
  ...
  public evaluate(context: AIContext_21, deltaTime: number): NodeStatus_21 {
    if (context.health <= 0) return NodeStatus_21.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_21, dt: number): NodeStatus_21 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_21.RUNNING;
  }

  private executeCombatLogic(context: AIContext_21, dt: number): NodeStatus_21 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_21.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_21, dt: number): NodeStatus_21 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_21.RUNNING;
  }

  private executePatrolLogic(context: AIContext_21, dt: number): NodeStatus_21 {
