...
export enum NodeStatus_3 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_3 {
  public name: string;
  public children: BehaviorNode_3[] = [];
  ...
  public evaluate(context: AIContext_3, deltaTime: number): NodeStatus_3 {
    if (context.health <= 0) return NodeStatus_3.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_3, dt: number): NodeStatus_3 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_3.RUNNING;
  }

  private executeCombatLogic(context: AIContext_3, dt: number): NodeStatus_3 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_3.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_3, dt: number): NodeStatus_3 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_3.RUNNING;
  }

  private executePatrolLogic(context: AIContext_3, dt: number): NodeStatus_3 {
