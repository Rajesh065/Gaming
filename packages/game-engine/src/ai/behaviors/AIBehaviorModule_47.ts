...
export enum NodeStatus_47 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_47 {
  public name: string;
  public children: BehaviorNode_47[] = [];
  ...
  public evaluate(context: AIContext_47, deltaTime: number): NodeStatus_47 {
    if (context.health <= 0) return NodeStatus_47.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_47, dt: number): NodeStatus_47 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_47.RUNNING;
  }

  private executeCombatLogic(context: AIContext_47, dt: number): NodeStatus_47 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_47.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_47, dt: number): NodeStatus_47 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_47.RUNNING;
  }

  private executePatrolLogic(context: AIContext_47, dt: number): NodeStatus_47 {
