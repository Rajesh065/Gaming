...
export enum NodeStatus_93 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_93 {
  public name: string;
  public children: BehaviorNode_93[] = [];
  ...
  public evaluate(context: AIContext_93, deltaTime: number): NodeStatus_93 {
    if (context.health <= 0) return NodeStatus_93.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_93, dt: number): NodeStatus_93 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_93.RUNNING;
  }

  private executeCombatLogic(context: AIContext_93, dt: number): NodeStatus_93 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_93.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_93, dt: number): NodeStatus_93 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_93.RUNNING;
  }

  private executePatrolLogic(context: AIContext_93, dt: number): NodeStatus_93 {
