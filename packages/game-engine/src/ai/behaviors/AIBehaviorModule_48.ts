...
export enum NodeStatus_48 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_48 {
  public name: string;
  public children: BehaviorNode_48[] = [];
  ...
  public evaluate(context: AIContext_48, deltaTime: number): NodeStatus_48 {
    if (context.health <= 0) return NodeStatus_48.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_48, dt: number): NodeStatus_48 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_48.RUNNING;
  }

  private executeCombatLogic(context: AIContext_48, dt: number): NodeStatus_48 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_48.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_48, dt: number): NodeStatus_48 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_48.RUNNING;
  }

  private executePatrolLogic(context: AIContext_48, dt: number): NodeStatus_48 {
