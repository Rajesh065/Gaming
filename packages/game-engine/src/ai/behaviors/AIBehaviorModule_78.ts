...
export enum NodeStatus_78 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_78 {
  public name: string;
  public children: BehaviorNode_78[] = [];
  ...
  public evaluate(context: AIContext_78, deltaTime: number): NodeStatus_78 {
    if (context.health <= 0) return NodeStatus_78.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_78, dt: number): NodeStatus_78 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_78.RUNNING;
  }

  private executeCombatLogic(context: AIContext_78, dt: number): NodeStatus_78 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_78.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_78, dt: number): NodeStatus_78 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_78.RUNNING;
  }

  private executePatrolLogic(context: AIContext_78, dt: number): NodeStatus_78 {
