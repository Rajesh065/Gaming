...
export enum NodeStatus_19 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_19 {
  public name: string;
  public children: BehaviorNode_19[] = [];
  ...
  public evaluate(context: AIContext_19, deltaTime: number): NodeStatus_19 {
    if (context.health <= 0) return NodeStatus_19.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_19, dt: number): NodeStatus_19 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_19.RUNNING;
  }

  private executeCombatLogic(context: AIContext_19, dt: number): NodeStatus_19 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_19.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_19, dt: number): NodeStatus_19 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_19.RUNNING;
  }

  private executePatrolLogic(context: AIContext_19, dt: number): NodeStatus_19 {
