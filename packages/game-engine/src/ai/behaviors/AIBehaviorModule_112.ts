...
export interface AIContext_112 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_112 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_112 {
  public name: string;
  public children: BehaviorNode_112[] = [];
  ...
  public evaluate(context: AIContext_112, deltaTime: number): NodeStatus_112 {
    if (context.health <= 0) return NodeStatus_112.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_112, dt: number): NodeStatus_112 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_112.RUNNING;
  }

  private executeCombatLogic(context: AIContext_112, dt: number): NodeStatus_112 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_112.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_112, dt: number): NodeStatus_112 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_112.RUNNING;
  }

  private executePatrolLogic(context: AIContext_112, dt: number): NodeStatus_112 {
