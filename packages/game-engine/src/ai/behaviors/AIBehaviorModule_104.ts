...
export interface AIContext_104 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_104 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_104 {
  public name: string;
  public children: BehaviorNode_104[] = [];
  ...
  public evaluate(context: AIContext_104, deltaTime: number): NodeStatus_104 {
    if (context.health <= 0) return NodeStatus_104.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_104, dt: number): NodeStatus_104 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_104.RUNNING;
  }

  private executeCombatLogic(context: AIContext_104, dt: number): NodeStatus_104 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_104.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_104, dt: number): NodeStatus_104 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_104.RUNNING;
  }

  private executePatrolLogic(context: AIContext_104, dt: number): NodeStatus_104 {
