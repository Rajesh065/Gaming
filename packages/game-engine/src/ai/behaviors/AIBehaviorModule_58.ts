...
export enum NodeStatus_58 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_58 {
  public name: string;
  public children: BehaviorNode_58[] = [];
  ...
  public evaluate(context: AIContext_58, deltaTime: number): NodeStatus_58 {
    if (context.health <= 0) return NodeStatus_58.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_58, dt: number): NodeStatus_58 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_58.RUNNING;
  }

  private executeCombatLogic(context: AIContext_58, dt: number): NodeStatus_58 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_58.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_58, dt: number): NodeStatus_58 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_58.RUNNING;
  }

  private executePatrolLogic(context: AIContext_58, dt: number): NodeStatus_58 {
