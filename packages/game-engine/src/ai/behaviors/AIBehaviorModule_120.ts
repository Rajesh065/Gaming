...
export interface AIContext_120 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_120 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_120 {
  public name: string;
  public children: BehaviorNode_120[] = [];
  ...
  public evaluate(context: AIContext_120, deltaTime: number): NodeStatus_120 {
    if (context.health <= 0) return NodeStatus_120.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_120, dt: number): NodeStatus_120 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_120.RUNNING;
  }

  private executeCombatLogic(context: AIContext_120, dt: number): NodeStatus_120 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_120.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_120, dt: number): NodeStatus_120 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_120.RUNNING;
  }

  private executePatrolLogic(context: AIContext_120, dt: number): NodeStatus_120 {
