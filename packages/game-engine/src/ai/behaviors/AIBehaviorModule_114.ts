...
export interface AIContext_114 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_114 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_114 {
  public name: string;
  public children: BehaviorNode_114[] = [];
  ...
  public evaluate(context: AIContext_114, deltaTime: number): NodeStatus_114 {
    if (context.health <= 0) return NodeStatus_114.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_114, dt: number): NodeStatus_114 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_114.RUNNING;
  }

  private executeCombatLogic(context: AIContext_114, dt: number): NodeStatus_114 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_114.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_114, dt: number): NodeStatus_114 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_114.RUNNING;
  }

  private executePatrolLogic(context: AIContext_114, dt: number): NodeStatus_114 {
