...
export enum NodeStatus_49 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_49 {
  public name: string;
  public children: BehaviorNode_49[] = [];
  ...
  public evaluate(context: AIContext_49, deltaTime: number): NodeStatus_49 {
    if (context.health <= 0) return NodeStatus_49.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_49, dt: number): NodeStatus_49 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_49.RUNNING;
  }

  private executeCombatLogic(context: AIContext_49, dt: number): NodeStatus_49 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_49.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_49, dt: number): NodeStatus_49 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_49.RUNNING;
  }

  private executePatrolLogic(context: AIContext_49, dt: number): NodeStatus_49 {
