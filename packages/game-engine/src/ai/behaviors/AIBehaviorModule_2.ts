...
export enum NodeStatus_2 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_2 {
  public name: string;
  public children: BehaviorNode_2[] = [];
  ...
  public evaluate(context: AIContext_2, deltaTime: number): NodeStatus_2 {
    if (context.health <= 0) return NodeStatus_2.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_2, dt: number): NodeStatus_2 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_2.RUNNING;
  }

  private executeCombatLogic(context: AIContext_2, dt: number): NodeStatus_2 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_2.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_2, dt: number): NodeStatus_2 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_2.RUNNING;
  }

  private executePatrolLogic(context: AIContext_2, dt: number): NodeStatus_2 {
