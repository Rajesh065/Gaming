...
export enum NodeStatus_77 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_77 {
  public name: string;
  public children: BehaviorNode_77[] = [];
  ...
  public evaluate(context: AIContext_77, deltaTime: number): NodeStatus_77 {
    if (context.health <= 0) return NodeStatus_77.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_77, dt: number): NodeStatus_77 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_77.RUNNING;
  }

  private executeCombatLogic(context: AIContext_77, dt: number): NodeStatus_77 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_77.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_77, dt: number): NodeStatus_77 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_77.RUNNING;
  }

  private executePatrolLogic(context: AIContext_77, dt: number): NodeStatus_77 {
