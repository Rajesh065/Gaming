...
export enum NodeStatus_59 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_59 {
  public name: string;
  public children: BehaviorNode_59[] = [];
  ...
  public evaluate(context: AIContext_59, deltaTime: number): NodeStatus_59 {
    if (context.health <= 0) return NodeStatus_59.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_59, dt: number): NodeStatus_59 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_59.RUNNING;
  }

  private executeCombatLogic(context: AIContext_59, dt: number): NodeStatus_59 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_59.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_59, dt: number): NodeStatus_59 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_59.RUNNING;
  }

  private executePatrolLogic(context: AIContext_59, dt: number): NodeStatus_59 {
