...
export interface AIContext_116 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_116 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_116 {
  public name: string;
  public children: BehaviorNode_116[] = [];
  ...
  public evaluate(context: AIContext_116, deltaTime: number): NodeStatus_116 {
    if (context.health <= 0) return NodeStatus_116.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_116, dt: number): NodeStatus_116 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_116.RUNNING;
  }

  private executeCombatLogic(context: AIContext_116, dt: number): NodeStatus_116 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_116.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_116, dt: number): NodeStatus_116 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_116.RUNNING;
  }

  private executePatrolLogic(context: AIContext_116, dt: number): NodeStatus_116 {
