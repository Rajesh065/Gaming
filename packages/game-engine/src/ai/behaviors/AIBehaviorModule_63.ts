...
export enum NodeStatus_63 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_63 {
  public name: string;
  public children: BehaviorNode_63[] = [];
  ...
  public evaluate(context: AIContext_63, deltaTime: number): NodeStatus_63 {
    if (context.health <= 0) return NodeStatus_63.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_63, dt: number): NodeStatus_63 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_63.RUNNING;
  }

  private executeCombatLogic(context: AIContext_63, dt: number): NodeStatus_63 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_63.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_63, dt: number): NodeStatus_63 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_63.RUNNING;
  }

  private executePatrolLogic(context: AIContext_63, dt: number): NodeStatus_63 {
