...
export interface AIContext_106 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_106 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_106 {
  public name: string;
  public children: BehaviorNode_106[] = [];
  ...
  public evaluate(context: AIContext_106, deltaTime: number): NodeStatus_106 {
    if (context.health <= 0) return NodeStatus_106.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_106, dt: number): NodeStatus_106 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_106.RUNNING;
  }

  private executeCombatLogic(context: AIContext_106, dt: number): NodeStatus_106 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_106.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_106, dt: number): NodeStatus_106 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_106.RUNNING;
  }

  private executePatrolLogic(context: AIContext_106, dt: number): NodeStatus_106 {
