...
export enum NodeStatus_91 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_91 {
  public name: string;
  public children: BehaviorNode_91[] = [];
  ...
  public evaluate(context: AIContext_91, deltaTime: number): NodeStatus_91 {
    if (context.health <= 0) return NodeStatus_91.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_91, dt: number): NodeStatus_91 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_91.RUNNING;
  }

  private executeCombatLogic(context: AIContext_91, dt: number): NodeStatus_91 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_91.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_91, dt: number): NodeStatus_91 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_91.RUNNING;
  }

  private executePatrolLogic(context: AIContext_91, dt: number): NodeStatus_91 {
