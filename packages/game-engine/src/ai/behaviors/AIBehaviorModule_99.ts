...
export enum NodeStatus_99 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_99 {
  public name: string;
  public children: BehaviorNode_99[] = [];
  ...
  public evaluate(context: AIContext_99, deltaTime: number): NodeStatus_99 {
    if (context.health <= 0) return NodeStatus_99.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_99, dt: number): NodeStatus_99 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_99.RUNNING;
  }

  private executeCombatLogic(context: AIContext_99, dt: number): NodeStatus_99 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_99.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_99, dt: number): NodeStatus_99 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_99.RUNNING;
  }

  private executePatrolLogic(context: AIContext_99, dt: number): NodeStatus_99 {
