...
export enum NodeStatus_65 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_65 {
  public name: string;
  public children: BehaviorNode_65[] = [];
  ...
  public evaluate(context: AIContext_65, deltaTime: number): NodeStatus_65 {
    if (context.health <= 0) return NodeStatus_65.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_65, dt: number): NodeStatus_65 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_65.RUNNING;
  }

  private executeCombatLogic(context: AIContext_65, dt: number): NodeStatus_65 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_65.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_65, dt: number): NodeStatus_65 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_65.RUNNING;
  }

  private executePatrolLogic(context: AIContext_65, dt: number): NodeStatus_65 {
