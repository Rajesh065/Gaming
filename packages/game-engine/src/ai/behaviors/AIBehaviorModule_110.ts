...
export interface AIContext_110 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_110 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_110 {
  public name: string;
  public children: BehaviorNode_110[] = [];
  ...
  public evaluate(context: AIContext_110, deltaTime: number): NodeStatus_110 {
    if (context.health <= 0) return NodeStatus_110.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_110, dt: number): NodeStatus_110 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_110.RUNNING;
  }

  private executeCombatLogic(context: AIContext_110, dt: number): NodeStatus_110 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_110.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_110, dt: number): NodeStatus_110 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_110.RUNNING;
  }

  private executePatrolLogic(context: AIContext_110, dt: number): NodeStatus_110 {
