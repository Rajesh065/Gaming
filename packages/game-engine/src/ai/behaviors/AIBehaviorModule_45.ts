...
export enum NodeStatus_45 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_45 {
  public name: string;
  public children: BehaviorNode_45[] = [];
  ...
  public evaluate(context: AIContext_45, deltaTime: number): NodeStatus_45 {
    if (context.health <= 0) return NodeStatus_45.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_45, dt: number): NodeStatus_45 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_45.RUNNING;
  }

  private executeCombatLogic(context: AIContext_45, dt: number): NodeStatus_45 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_45.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_45, dt: number): NodeStatus_45 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_45.RUNNING;
  }

  private executePatrolLogic(context: AIContext_45, dt: number): NodeStatus_45 {
