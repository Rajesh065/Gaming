...
export enum NodeStatus_54 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_54 {
  public name: string;
  public children: BehaviorNode_54[] = [];
  ...
  public evaluate(context: AIContext_54, deltaTime: number): NodeStatus_54 {
    if (context.health <= 0) return NodeStatus_54.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_54, dt: number): NodeStatus_54 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_54.RUNNING;
  }

  private executeCombatLogic(context: AIContext_54, dt: number): NodeStatus_54 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_54.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_54, dt: number): NodeStatus_54 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_54.RUNNING;
  }

  private executePatrolLogic(context: AIContext_54, dt: number): NodeStatus_54 {
