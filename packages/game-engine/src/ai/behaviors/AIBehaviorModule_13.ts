...
export enum NodeStatus_13 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_13 {
  public name: string;
  public children: BehaviorNode_13[] = [];
  ...
  public evaluate(context: AIContext_13, deltaTime: number): NodeStatus_13 {
    if (context.health <= 0) return NodeStatus_13.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_13, dt: number): NodeStatus_13 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_13.RUNNING;
  }

  private executeCombatLogic(context: AIContext_13, dt: number): NodeStatus_13 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_13.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_13, dt: number): NodeStatus_13 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_13.RUNNING;
  }

  private executePatrolLogic(context: AIContext_13, dt: number): NodeStatus_13 {
