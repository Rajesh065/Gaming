...
export enum NodeStatus_56 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_56 {
  public name: string;
  public children: BehaviorNode_56[] = [];
  ...
  public evaluate(context: AIContext_56, deltaTime: number): NodeStatus_56 {
    if (context.health <= 0) return NodeStatus_56.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_56, dt: number): NodeStatus_56 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_56.RUNNING;
  }

  private executeCombatLogic(context: AIContext_56, dt: number): NodeStatus_56 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_56.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_56, dt: number): NodeStatus_56 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_56.RUNNING;
  }

  private executePatrolLogic(context: AIContext_56, dt: number): NodeStatus_56 {
