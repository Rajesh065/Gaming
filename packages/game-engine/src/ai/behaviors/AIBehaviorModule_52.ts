...
export enum NodeStatus_52 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_52 {
  public name: string;
  public children: BehaviorNode_52[] = [];
  ...
  public evaluate(context: AIContext_52, deltaTime: number): NodeStatus_52 {
    if (context.health <= 0) return NodeStatus_52.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_52, dt: number): NodeStatus_52 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_52.RUNNING;
  }

  private executeCombatLogic(context: AIContext_52, dt: number): NodeStatus_52 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_52.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_52, dt: number): NodeStatus_52 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_52.RUNNING;
  }

  private executePatrolLogic(context: AIContext_52, dt: number): NodeStatus_52 {
