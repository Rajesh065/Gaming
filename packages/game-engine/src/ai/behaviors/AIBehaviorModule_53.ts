...
export enum NodeStatus_53 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_53 {
  public name: string;
  public children: BehaviorNode_53[] = [];
  ...
  public evaluate(context: AIContext_53, deltaTime: number): NodeStatus_53 {
    if (context.health <= 0) return NodeStatus_53.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_53, dt: number): NodeStatus_53 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_53.RUNNING;
  }

  private executeCombatLogic(context: AIContext_53, dt: number): NodeStatus_53 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_53.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_53, dt: number): NodeStatus_53 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_53.RUNNING;
  }

  private executePatrolLogic(context: AIContext_53, dt: number): NodeStatus_53 {
