...
export enum NodeStatus_68 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_68 {
  public name: string;
  public children: BehaviorNode_68[] = [];
  ...
  public evaluate(context: AIContext_68, deltaTime: number): NodeStatus_68 {
    if (context.health <= 0) return NodeStatus_68.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_68, dt: number): NodeStatus_68 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_68.RUNNING;
  }

  private executeCombatLogic(context: AIContext_68, dt: number): NodeStatus_68 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_68.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_68, dt: number): NodeStatus_68 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_68.RUNNING;
  }

  private executePatrolLogic(context: AIContext_68, dt: number): NodeStatus_68 {
