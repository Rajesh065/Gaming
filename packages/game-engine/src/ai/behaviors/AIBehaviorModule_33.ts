...
export enum NodeStatus_33 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_33 {
  public name: string;
  public children: BehaviorNode_33[] = [];
  ...
  public evaluate(context: AIContext_33, deltaTime: number): NodeStatus_33 {
    if (context.health <= 0) return NodeStatus_33.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_33, dt: number): NodeStatus_33 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_33.RUNNING;
  }

  private executeCombatLogic(context: AIContext_33, dt: number): NodeStatus_33 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_33.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_33, dt: number): NodeStatus_33 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_33.RUNNING;
  }

  private executePatrolLogic(context: AIContext_33, dt: number): NodeStatus_33 {
