...
export enum NodeStatus_38 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_38 {
  public name: string;
  public children: BehaviorNode_38[] = [];
  ...
  public evaluate(context: AIContext_38, deltaTime: number): NodeStatus_38 {
    if (context.health <= 0) return NodeStatus_38.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_38, dt: number): NodeStatus_38 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_38.RUNNING;
  }

  private executeCombatLogic(context: AIContext_38, dt: number): NodeStatus_38 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_38.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_38, dt: number): NodeStatus_38 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_38.RUNNING;
  }

  private executePatrolLogic(context: AIContext_38, dt: number): NodeStatus_38 {
