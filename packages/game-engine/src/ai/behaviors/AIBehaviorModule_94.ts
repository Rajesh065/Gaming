...
export enum NodeStatus_94 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_94 {
  public name: string;
  public children: BehaviorNode_94[] = [];
  ...
  public evaluate(context: AIContext_94, deltaTime: number): NodeStatus_94 {
    if (context.health <= 0) return NodeStatus_94.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_94, dt: number): NodeStatus_94 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_94.RUNNING;
  }

  private executeCombatLogic(context: AIContext_94, dt: number): NodeStatus_94 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_94.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_94, dt: number): NodeStatus_94 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_94.RUNNING;
  }

  private executePatrolLogic(context: AIContext_94, dt: number): NodeStatus_94 {
