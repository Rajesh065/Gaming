...
export enum NodeStatus_80 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_80 {
  public name: string;
  public children: BehaviorNode_80[] = [];
  ...
  public evaluate(context: AIContext_80, deltaTime: number): NodeStatus_80 {
    if (context.health <= 0) return NodeStatus_80.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_80, dt: number): NodeStatus_80 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_80.RUNNING;
  }

  private executeCombatLogic(context: AIContext_80, dt: number): NodeStatus_80 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_80.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_80, dt: number): NodeStatus_80 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_80.RUNNING;
  }

  private executePatrolLogic(context: AIContext_80, dt: number): NodeStatus_80 {
