...
export enum NodeStatus_95 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_95 {
  public name: string;
  public children: BehaviorNode_95[] = [];
  ...
  public evaluate(context: AIContext_95, deltaTime: number): NodeStatus_95 {
    if (context.health <= 0) return NodeStatus_95.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_95, dt: number): NodeStatus_95 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_95.RUNNING;
  }

  private executeCombatLogic(context: AIContext_95, dt: number): NodeStatus_95 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_95.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_95, dt: number): NodeStatus_95 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_95.RUNNING;
  }

  private executePatrolLogic(context: AIContext_95, dt: number): NodeStatus_95 {
