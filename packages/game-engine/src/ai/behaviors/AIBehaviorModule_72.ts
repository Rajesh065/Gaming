...
export enum NodeStatus_72 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_72 {
  public name: string;
  public children: BehaviorNode_72[] = [];
  ...
  public evaluate(context: AIContext_72, deltaTime: number): NodeStatus_72 {
    if (context.health <= 0) return NodeStatus_72.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_72, dt: number): NodeStatus_72 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_72.RUNNING;
  }

  private executeCombatLogic(context: AIContext_72, dt: number): NodeStatus_72 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_72.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_72, dt: number): NodeStatus_72 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_72.RUNNING;
  }

  private executePatrolLogic(context: AIContext_72, dt: number): NodeStatus_72 {
