...
export enum NodeStatus_85 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_85 {
  public name: string;
  public children: BehaviorNode_85[] = [];
  ...
  public evaluate(context: AIContext_85, deltaTime: number): NodeStatus_85 {
    if (context.health <= 0) return NodeStatus_85.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_85, dt: number): NodeStatus_85 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_85.RUNNING;
  }

  private executeCombatLogic(context: AIContext_85, dt: number): NodeStatus_85 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_85.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_85, dt: number): NodeStatus_85 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_85.RUNNING;
  }

  private executePatrolLogic(context: AIContext_85, dt: number): NodeStatus_85 {
