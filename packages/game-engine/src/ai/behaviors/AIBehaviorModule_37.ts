...
export enum NodeStatus_37 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_37 {
  public name: string;
  public children: BehaviorNode_37[] = [];
  ...
  public evaluate(context: AIContext_37, deltaTime: number): NodeStatus_37 {
    if (context.health <= 0) return NodeStatus_37.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_37, dt: number): NodeStatus_37 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_37.RUNNING;
  }

  private executeCombatLogic(context: AIContext_37, dt: number): NodeStatus_37 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_37.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_37, dt: number): NodeStatus_37 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_37.RUNNING;
  }

  private executePatrolLogic(context: AIContext_37, dt: number): NodeStatus_37 {
