...
export enum NodeStatus_23 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_23 {
  public name: string;
  public children: BehaviorNode_23[] = [];
  ...
  public evaluate(context: AIContext_23, deltaTime: number): NodeStatus_23 {
    if (context.health <= 0) return NodeStatus_23.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_23, dt: number): NodeStatus_23 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_23.RUNNING;
  }

  private executeCombatLogic(context: AIContext_23, dt: number): NodeStatus_23 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_23.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_23, dt: number): NodeStatus_23 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_23.RUNNING;
  }

  private executePatrolLogic(context: AIContext_23, dt: number): NodeStatus_23 {
