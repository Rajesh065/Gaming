...
export enum NodeStatus_96 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_96 {
  public name: string;
  public children: BehaviorNode_96[] = [];
  ...
  public evaluate(context: AIContext_96, deltaTime: number): NodeStatus_96 {
    if (context.health <= 0) return NodeStatus_96.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_96, dt: number): NodeStatus_96 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_96.RUNNING;
  }

  private executeCombatLogic(context: AIContext_96, dt: number): NodeStatus_96 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_96.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_96, dt: number): NodeStatus_96 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_96.RUNNING;
  }

  private executePatrolLogic(context: AIContext_96, dt: number): NodeStatus_96 {
