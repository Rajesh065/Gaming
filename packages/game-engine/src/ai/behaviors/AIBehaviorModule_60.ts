...
export enum NodeStatus_60 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_60 {
  public name: string;
  public children: BehaviorNode_60[] = [];
  ...
  public evaluate(context: AIContext_60, deltaTime: number): NodeStatus_60 {
    if (context.health <= 0) return NodeStatus_60.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_60, dt: number): NodeStatus_60 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_60.RUNNING;
  }

  private executeCombatLogic(context: AIContext_60, dt: number): NodeStatus_60 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_60.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_60, dt: number): NodeStatus_60 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_60.RUNNING;
  }

  private executePatrolLogic(context: AIContext_60, dt: number): NodeStatus_60 {
