...
export enum NodeStatus_46 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_46 {
  public name: string;
  public children: BehaviorNode_46[] = [];
  ...
  public evaluate(context: AIContext_46, deltaTime: number): NodeStatus_46 {
    if (context.health <= 0) return NodeStatus_46.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_46, dt: number): NodeStatus_46 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_46.RUNNING;
  }

  private executeCombatLogic(context: AIContext_46, dt: number): NodeStatus_46 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_46.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_46, dt: number): NodeStatus_46 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_46.RUNNING;
  }

  private executePatrolLogic(context: AIContext_46, dt: number): NodeStatus_46 {
