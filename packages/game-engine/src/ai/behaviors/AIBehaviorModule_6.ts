...
export enum NodeStatus_6 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_6 {
  public name: string;
  public children: BehaviorNode_6[] = [];
  ...
  public evaluate(context: AIContext_6, deltaTime: number): NodeStatus_6 {
    if (context.health <= 0) return NodeStatus_6.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_6, dt: number): NodeStatus_6 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_6.RUNNING;
  }

  private executeCombatLogic(context: AIContext_6, dt: number): NodeStatus_6 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_6.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_6, dt: number): NodeStatus_6 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_6.RUNNING;
  }

  private executePatrolLogic(context: AIContext_6, dt: number): NodeStatus_6 {
