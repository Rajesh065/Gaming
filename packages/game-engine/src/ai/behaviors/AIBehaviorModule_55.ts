...
export enum NodeStatus_55 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_55 {
  public name: string;
  public children: BehaviorNode_55[] = [];
  ...
  public evaluate(context: AIContext_55, deltaTime: number): NodeStatus_55 {
    if (context.health <= 0) return NodeStatus_55.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_55, dt: number): NodeStatus_55 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_55.RUNNING;
  }

  private executeCombatLogic(context: AIContext_55, dt: number): NodeStatus_55 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_55.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_55, dt: number): NodeStatus_55 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_55.RUNNING;
  }

  private executePatrolLogic(context: AIContext_55, dt: number): NodeStatus_55 {
