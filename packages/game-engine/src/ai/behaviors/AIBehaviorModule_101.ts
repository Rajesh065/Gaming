...
export interface AIContext_101 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_101 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_101 {
  public name: string;
  public children: BehaviorNode_101[] = [];
  ...
  public evaluate(context: AIContext_101, deltaTime: number): NodeStatus_101 {
    if (context.health <= 0) return NodeStatus_101.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_101, dt: number): NodeStatus_101 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_101.RUNNING;
  }

  private executeCombatLogic(context: AIContext_101, dt: number): NodeStatus_101 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_101.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_101, dt: number): NodeStatus_101 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_101.RUNNING;
  }

  private executePatrolLogic(context: AIContext_101, dt: number): NodeStatus_101 {
