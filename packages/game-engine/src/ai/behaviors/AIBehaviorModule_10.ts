...
export enum NodeStatus_10 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_10 {
  public name: string;
  public children: BehaviorNode_10[] = [];
  ...
  public evaluate(context: AIContext_10, deltaTime: number): NodeStatus_10 {
    if (context.health <= 0) return NodeStatus_10.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_10, dt: number): NodeStatus_10 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_10.RUNNING;
  }

  private executeCombatLogic(context: AIContext_10, dt: number): NodeStatus_10 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_10.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_10, dt: number): NodeStatus_10 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_10.RUNNING;
  }

  private executePatrolLogic(context: AIContext_10, dt: number): NodeStatus_10 {
