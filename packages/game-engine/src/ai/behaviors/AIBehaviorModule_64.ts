...
export enum NodeStatus_64 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_64 {
  public name: string;
  public children: BehaviorNode_64[] = [];
  ...
  public evaluate(context: AIContext_64, deltaTime: number): NodeStatus_64 {
    if (context.health <= 0) return NodeStatus_64.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_64, dt: number): NodeStatus_64 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_64.RUNNING;
  }

  private executeCombatLogic(context: AIContext_64, dt: number): NodeStatus_64 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_64.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_64, dt: number): NodeStatus_64 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_64.RUNNING;
  }

  private executePatrolLogic(context: AIContext_64, dt: number): NodeStatus_64 {
