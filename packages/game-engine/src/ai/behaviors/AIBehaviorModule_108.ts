...
export interface AIContext_108 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_108 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_108 {
  public name: string;
  public children: BehaviorNode_108[] = [];
  ...
  public evaluate(context: AIContext_108, deltaTime: number): NodeStatus_108 {
    if (context.health <= 0) return NodeStatus_108.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_108, dt: number): NodeStatus_108 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_108.RUNNING;
  }

  private executeCombatLogic(context: AIContext_108, dt: number): NodeStatus_108 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_108.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_108, dt: number): NodeStatus_108 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_108.RUNNING;
  }

  private executePatrolLogic(context: AIContext_108, dt: number): NodeStatus_108 {
