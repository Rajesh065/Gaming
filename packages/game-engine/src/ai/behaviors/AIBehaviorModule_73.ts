...
export enum NodeStatus_73 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_73 {
  public name: string;
  public children: BehaviorNode_73[] = [];
  ...
  public evaluate(context: AIContext_73, deltaTime: number): NodeStatus_73 {
    if (context.health <= 0) return NodeStatus_73.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_73, dt: number): NodeStatus_73 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_73.RUNNING;
  }

  private executeCombatLogic(context: AIContext_73, dt: number): NodeStatus_73 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_73.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_73, dt: number): NodeStatus_73 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_73.RUNNING;
  }

  private executePatrolLogic(context: AIContext_73, dt: number): NodeStatus_73 {
