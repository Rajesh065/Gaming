...
export enum NodeStatus_8 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_8 {
  public name: string;
  public children: BehaviorNode_8[] = [];
  ...
  public evaluate(context: AIContext_8, deltaTime: number): NodeStatus_8 {
    if (context.health <= 0) return NodeStatus_8.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_8, dt: number): NodeStatus_8 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_8.RUNNING;
  }

  private executeCombatLogic(context: AIContext_8, dt: number): NodeStatus_8 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_8.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_8, dt: number): NodeStatus_8 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_8.RUNNING;
  }

  private executePatrolLogic(context: AIContext_8, dt: number): NodeStatus_8 {
