...
export enum NodeStatus_34 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_34 {
  public name: string;
  public children: BehaviorNode_34[] = [];
  ...
  public evaluate(context: AIContext_34, deltaTime: number): NodeStatus_34 {
    if (context.health <= 0) return NodeStatus_34.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_34, dt: number): NodeStatus_34 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_34.RUNNING;
  }

  private executeCombatLogic(context: AIContext_34, dt: number): NodeStatus_34 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_34.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_34, dt: number): NodeStatus_34 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_34.RUNNING;
  }

  private executePatrolLogic(context: AIContext_34, dt: number): NodeStatus_34 {
