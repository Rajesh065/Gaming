...
export enum NodeStatus_27 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_27 {
  public name: string;
  public children: BehaviorNode_27[] = [];
  ...
  public evaluate(context: AIContext_27, deltaTime: number): NodeStatus_27 {
    if (context.health <= 0) return NodeStatus_27.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_27, dt: number): NodeStatus_27 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_27.RUNNING;
  }

  private executeCombatLogic(context: AIContext_27, dt: number): NodeStatus_27 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_27.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_27, dt: number): NodeStatus_27 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_27.RUNNING;
  }

  private executePatrolLogic(context: AIContext_27, dt: number): NodeStatus_27 {
