...
export interface AIContext_107 {
  entityId: string;
  targetId?: string;
  health: number;
  maxHealth: number;
...
export enum NodeStatus_107 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_107 {
  public name: string;
  public children: BehaviorNode_107[] = [];
  ...
  public evaluate(context: AIContext_107, deltaTime: number): NodeStatus_107 {
    if (context.health <= 0) return NodeStatus_107.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_107, dt: number): NodeStatus_107 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_107.RUNNING;
  }

  private executeCombatLogic(context: AIContext_107, dt: number): NodeStatus_107 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_107.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_107, dt: number): NodeStatus_107 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_107.RUNNING;
  }

  private executePatrolLogic(context: AIContext_107, dt: number): NodeStatus_107 {
