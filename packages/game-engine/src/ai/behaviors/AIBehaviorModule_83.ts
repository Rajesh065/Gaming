...
export enum NodeStatus_83 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_83 {
  public name: string;
  public children: BehaviorNode_83[] = [];
  ...
  public evaluate(context: AIContext_83, deltaTime: number): NodeStatus_83 {
    if (context.health <= 0) return NodeStatus_83.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_83, dt: number): NodeStatus_83 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_83.RUNNING;
  }

  private executeCombatLogic(context: AIContext_83, dt: number): NodeStatus_83 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_83.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_83, dt: number): NodeStatus_83 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_83.RUNNING;
  }

  private executePatrolLogic(context: AIContext_83, dt: number): NodeStatus_83 {
