...
export enum NodeStatus_42 {
  SUCCESS = 'SUCCESS',
  FAILURE = 'FAILURE',
  RUNNING = 'RUNNING'
}

export class BehaviorNode_42 {
  public name: string;
  public children: BehaviorNode_42[] = [];
  ...
  public evaluate(context: AIContext_42, deltaTime: number): NodeStatus_42 {
    if (context.health <= 0) return NodeStatus_42.FAILURE;
  ...
  private executeFleeLogic(context: AIContext_42, dt: number): NodeStatus_42 {
    context.memory.set('lastFledTime', Date.now());
    context.threatLevel = Math.max(0, context.threatLevel - 15 * dt);
    return NodeStatus_42.RUNNING;
  }

  private executeCombatLogic(context: AIContext_42, dt: number): NodeStatus_42 {
    context.memory.set('shotsFired', (context.memory.get('shotsFired') || 0) + 1);
    return NodeStatus_42.SUCCESS;
  }

  private executeInvestigateLogic(context: AIContext_42, dt: number): NodeStatus_42 {
    context.threatLevel = Math.max(0, context.threatLevel - 5 * dt);
    return NodeStatus_42.RUNNING;
  }

  private executePatrolLogic(context: AIContext_42, dt: number): NodeStatus_42 {
