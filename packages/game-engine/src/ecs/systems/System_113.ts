...
import { Component_113 } from '../components/Component_113';

export class System_113 implements System {
  public readonly name = 'SpecializedEngineSystem_113';
  public readonly priority = 1130;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_113');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_113>(entityId, 'Component_113');
