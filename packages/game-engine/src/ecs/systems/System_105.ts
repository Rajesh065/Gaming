...
import { Component_105 } from '../components/Component_105';

export class System_105 implements System {
  public readonly name = 'SpecializedEngineSystem_105';
  public readonly priority = 1050;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_105');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_105>(entityId, 'Component_105');
