...
import { Component_114 } from '../components/Component_114';

export class System_114 implements System {
  public readonly name = 'SpecializedEngineSystem_114';
  public readonly priority = 1140;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_114');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_114>(entityId, 'Component_114');
