...
export class System_36 implements System {
  public readonly name = 'SpecializedEngineSystem_36';
  public readonly priority = 360;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_36');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_36>(entityId, 'Component_36');
