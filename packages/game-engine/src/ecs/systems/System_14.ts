...
export class System_14 implements System {
  public readonly name = 'SpecializedEngineSystem_14';
  public readonly priority = 140;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_14');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_14>(entityId, 'Component_14');
