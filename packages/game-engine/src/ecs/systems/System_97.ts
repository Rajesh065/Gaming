...
import { Component_97 } from '../components/Component_97';

export class System_97 implements System {
  public readonly name = 'SpecializedEngineSystem_97';
  public readonly priority = 970;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_97');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_97>(entityId, 'Component_97');
