...
import { Component_111 } from '../components/Component_111';

export class System_111 implements System {
  public readonly name = 'SpecializedEngineSystem_111';
  public readonly priority = 1110;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_111');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_111>(entityId, 'Component_111');
