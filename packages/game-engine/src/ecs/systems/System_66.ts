...
import { Component_66 } from '../components/Component_66';

export class System_66 implements System {
  public readonly name = 'SpecializedEngineSystem_66';
  public readonly priority = 660;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_66');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_66>(entityId, 'Component_66');
