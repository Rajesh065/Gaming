...
export class System_9 implements System {
  public readonly name = 'SpecializedEngineSystem_9';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_9');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_9>(entityId, 'Component_9');
