...
import { Component_120 } from '../components/Component_120';

export class System_120 implements System {
  public readonly name = 'SpecializedEngineSystem_120';
  public readonly priority = 1200;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_120');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_120>(entityId, 'Component_120');
