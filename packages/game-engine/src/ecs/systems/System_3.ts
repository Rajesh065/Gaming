...
export class System_3 implements System {
  public readonly name = 'SpecializedEngineSystem_3';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_3');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_3>(entityId, 'Component_3');
