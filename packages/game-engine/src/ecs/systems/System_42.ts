...
import { Component_42 } from '../components/Component_42';

export class System_42 implements System {
  public readonly name = 'SpecializedEngineSystem_42';
  public readonly priority = 420;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_42');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_42>(entityId, 'Component_42');
