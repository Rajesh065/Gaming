...
export class System_24 implements System {
  public readonly name = 'SpecializedEngineSystem_24';
  public readonly priority = 240;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_24');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_24>(entityId, 'Component_24');
