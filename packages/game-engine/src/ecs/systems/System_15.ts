...
export class System_15 implements System {
  public readonly name = 'SpecializedEngineSystem_15';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_15');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_15>(entityId, 'Component_15');
