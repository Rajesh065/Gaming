...
import { Component_106 } from '../components/Component_106';

export class System_106 implements System {
  public readonly name = 'SpecializedEngineSystem_106';
  public readonly priority = 1060;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_106');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_106>(entityId, 'Component_106');
