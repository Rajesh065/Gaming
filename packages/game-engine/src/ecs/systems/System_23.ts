...
export class System_23 implements System {
  public readonly name = 'SpecializedEngineSystem_23';
  public readonly priority = 230;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_23');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_23>(entityId, 'Component_23');
