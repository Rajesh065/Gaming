...
export class System_27 implements System {
  public readonly name = 'SpecializedEngineSystem_27';
  public readonly priority = 270;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_27');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_27>(entityId, 'Component_27');
