...
import { Component_80 } from '../components/Component_80';

export class System_80 implements System {
  public readonly name = 'SpecializedEngineSystem_80';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_80');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_80>(entityId, 'Component_80');
