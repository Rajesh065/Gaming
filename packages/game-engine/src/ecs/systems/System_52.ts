...
import { Component_52 } from '../components/Component_52';

export class System_52 implements System {
  public readonly name = 'SpecializedEngineSystem_52';
  public readonly priority = 520;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_52');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_52>(entityId, 'Component_52');
