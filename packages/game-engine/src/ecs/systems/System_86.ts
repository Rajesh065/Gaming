...
import { Component_86 } from '../components/Component_86';

export class System_86 implements System {
  public readonly name = 'SpecializedEngineSystem_86';
  public readonly priority = 860;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_86');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_86>(entityId, 'Component_86');
