...
export class System_34 implements System {
  public readonly name = 'SpecializedEngineSystem_34';
  public readonly priority = 340;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_34');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_34>(entityId, 'Component_34');
