...
import { Component_72 } from '../components/Component_72';

export class System_72 implements System {
  public readonly name = 'SpecializedEngineSystem_72';
  public readonly priority = 720;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_72');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_72>(entityId, 'Component_72');
