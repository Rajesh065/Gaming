...
export class System_25 implements System {
  public readonly name = 'SpecializedEngineSystem_25';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_25');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_25>(entityId, 'Component_25');
