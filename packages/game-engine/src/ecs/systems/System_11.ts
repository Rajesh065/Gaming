...
export class System_11 implements System {
  public readonly name = 'SpecializedEngineSystem_11';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_11');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_11>(entityId, 'Component_11');
