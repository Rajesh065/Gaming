...
import { Component_88 } from '../components/Component_88';

export class System_88 implements System {
  public readonly name = 'SpecializedEngineSystem_88';
  public readonly priority = 880;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_88');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_88>(entityId, 'Component_88');
