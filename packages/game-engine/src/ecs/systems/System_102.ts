...
import { Component_102 } from '../components/Component_102';

export class System_102 implements System {
  public readonly name = 'SpecializedEngineSystem_102';
  public readonly priority = 1020;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_102');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_102>(entityId, 'Component_102');
