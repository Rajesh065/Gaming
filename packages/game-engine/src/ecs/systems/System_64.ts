...
import { Component_64 } from '../components/Component_64';

export class System_64 implements System {
  public readonly name = 'SpecializedEngineSystem_64';
  public readonly priority = 640;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_64');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_64>(entityId, 'Component_64');
