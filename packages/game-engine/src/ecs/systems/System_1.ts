...
export class System_1 implements System {
  public readonly name = 'SpecializedEngineSystem_1';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_1');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_1>(entityId, 'Component_1');
