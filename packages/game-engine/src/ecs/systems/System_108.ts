...
import { Component_108 } from '../components/Component_108';

export class System_108 implements System {
  public readonly name = 'SpecializedEngineSystem_108';
  public readonly priority = 1080;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_108');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_108>(entityId, 'Component_108');
