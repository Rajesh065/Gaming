...
import { Component_73 } from '../components/Component_73';

export class System_73 implements System {
  public readonly name = 'SpecializedEngineSystem_73';
  public readonly priority = 730;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_73');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_73>(entityId, 'Component_73');
