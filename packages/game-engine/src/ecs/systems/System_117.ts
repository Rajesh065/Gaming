...
import { Component_117 } from '../components/Component_117';

export class System_117 implements System {
  public readonly name = 'SpecializedEngineSystem_117';
  public readonly priority = 1170;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_117');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_117>(entityId, 'Component_117');
