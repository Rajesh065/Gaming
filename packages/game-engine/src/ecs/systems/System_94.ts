...
import { Component_94 } from '../components/Component_94';

export class System_94 implements System {
  public readonly name = 'SpecializedEngineSystem_94';
  public readonly priority = 940;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_94');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_94>(entityId, 'Component_94');
