...
import { Component_84 } from '../components/Component_84';

export class System_84 implements System {
  public readonly name = 'SpecializedEngineSystem_84';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_84');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_84>(entityId, 'Component_84');
