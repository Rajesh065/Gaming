...
import { Component_65 } from '../components/Component_65';

export class System_65 implements System {
  public readonly name = 'SpecializedEngineSystem_65';
  public readonly priority = 650;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_65');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_65>(entityId, 'Component_65');
