...
import { Component_100 } from '../components/Component_100';

export class System_100 implements System {
  public readonly name = 'SpecializedEngineSystem_100';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_100');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_100>(entityId, 'Component_100');
