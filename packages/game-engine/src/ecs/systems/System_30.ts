...
export class System_30 implements System {
  public readonly name = 'SpecializedEngineSystem_30';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_30');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_30>(entityId, 'Component_30');
