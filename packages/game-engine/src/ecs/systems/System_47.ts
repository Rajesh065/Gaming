...
import { Component_47 } from '../components/Component_47';

export class System_47 implements System {
  public readonly name = 'SpecializedEngineSystem_47';
  public readonly priority = 470;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_47');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_47>(entityId, 'Component_47');
