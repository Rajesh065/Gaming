...
import { Component_116 } from '../components/Component_116';

export class System_116 implements System {
  public readonly name = 'SpecializedEngineSystem_116';
  public readonly priority = 1160;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_116');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_116>(entityId, 'Component_116');
