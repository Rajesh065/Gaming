...
export class System_31 implements System {
  public readonly name = 'SpecializedEngineSystem_31';
  public readonly priority = 310;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_31');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_31>(entityId, 'Component_31');
