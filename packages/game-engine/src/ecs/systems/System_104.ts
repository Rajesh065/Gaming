...
import { Component_104 } from '../components/Component_104';

export class System_104 implements System {
  public readonly name = 'SpecializedEngineSystem_104';
  public readonly priority = 1040;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_104');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_104>(entityId, 'Component_104');
