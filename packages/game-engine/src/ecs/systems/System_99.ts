...
import { Component_99 } from '../components/Component_99';

export class System_99 implements System {
  public readonly name = 'SpecializedEngineSystem_99';
  public readonly priority = 990;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_99');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_99>(entityId, 'Component_99');
