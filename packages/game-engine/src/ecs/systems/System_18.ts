...
export class System_18 implements System {
  public readonly name = 'SpecializedEngineSystem_18';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_18');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_18>(entityId, 'Component_18');
