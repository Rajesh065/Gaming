...
import { Component_56 } from '../components/Component_56';

export class System_56 implements System {
  public readonly name = 'SpecializedEngineSystem_56';
  public readonly priority = 560;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_56');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_56>(entityId, 'Component_56');
