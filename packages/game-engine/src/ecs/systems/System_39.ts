...
export class System_39 implements System {
  public readonly name = 'SpecializedEngineSystem_39';
  public readonly priority = 390;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_39');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_39>(entityId, 'Component_39');
