...
export class System_12 implements System {
  public readonly name = 'SpecializedEngineSystem_12';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_12');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_12>(entityId, 'Component_12');
