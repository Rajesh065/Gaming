...
export class System_38 implements System {
  public readonly name = 'SpecializedEngineSystem_38';
  public readonly priority = 380;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_38');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_38>(entityId, 'Component_38');
