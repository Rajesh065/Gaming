...
export class System_28 implements System {
  public readonly name = 'SpecializedEngineSystem_28';
  public readonly priority = 280;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_28');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_28>(entityId, 'Component_28');
