...
import { Component_45 } from '../components/Component_45';

export class System_45 implements System {
  public readonly name = 'SpecializedEngineSystem_45';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_45');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_45>(entityId, 'Component_45');
