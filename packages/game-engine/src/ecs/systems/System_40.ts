...
export class System_40 implements System {
  public readonly name = 'SpecializedEngineSystem_40';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_40');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_40>(entityId, 'Component_40');
