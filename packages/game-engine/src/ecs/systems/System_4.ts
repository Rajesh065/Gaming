...
export class System_4 implements System {
  public readonly name = 'SpecializedEngineSystem_4';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_4');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_4>(entityId, 'Component_4');
