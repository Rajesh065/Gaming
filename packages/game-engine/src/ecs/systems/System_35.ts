...
export class System_35 implements System {
  public readonly name = 'SpecializedEngineSystem_35';
  public readonly priority = 350;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_35');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_35>(entityId, 'Component_35');
