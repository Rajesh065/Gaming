...
import { Component_55 } from '../components/Component_55';

export class System_55 implements System {
  public readonly name = 'SpecializedEngineSystem_55';
  public readonly priority = 550;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_55');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_55>(entityId, 'Component_55');
