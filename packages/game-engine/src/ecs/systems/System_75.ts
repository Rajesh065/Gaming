...
import { Component_75 } from '../components/Component_75';

export class System_75 implements System {
  public readonly name = 'SpecializedEngineSystem_75';
  public readonly priority = 750;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_75');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_75>(entityId, 'Component_75');
