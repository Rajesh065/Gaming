...
import { Component_41 } from '../components/Component_41';

export class System_41 implements System {
  public readonly name = 'SpecializedEngineSystem_41';
  public readonly priority = 410;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_41');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_41>(entityId, 'Component_41');
