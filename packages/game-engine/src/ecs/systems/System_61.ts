...
import { Component_61 } from '../components/Component_61';

export class System_61 implements System {
  public readonly name = 'SpecializedEngineSystem_61';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_61');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_61>(entityId, 'Component_61');
