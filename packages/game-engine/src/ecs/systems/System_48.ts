...
import { Component_48 } from '../components/Component_48';

export class System_48 implements System {
  public readonly name = 'SpecializedEngineSystem_48';
  public readonly priority = 480;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_48');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_48>(entityId, 'Component_48');
