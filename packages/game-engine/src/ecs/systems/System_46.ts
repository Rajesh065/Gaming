...
import { Component_46 } from '../components/Component_46';

export class System_46 implements System {
  public readonly name = 'SpecializedEngineSystem_46';
  public readonly priority = 460;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_46');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_46>(entityId, 'Component_46');
