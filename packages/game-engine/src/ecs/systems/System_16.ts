...
export class System_16 implements System {
  public readonly name = 'SpecializedEngineSystem_16';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_16');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_16>(entityId, 'Component_16');
