...
import { Component_53 } from '../components/Component_53';

export class System_53 implements System {
  public readonly name = 'SpecializedEngineSystem_53';
  public readonly priority = 530;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_53');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_53>(entityId, 'Component_53');
