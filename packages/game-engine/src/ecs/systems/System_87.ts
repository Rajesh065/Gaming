...
import { Component_87 } from '../components/Component_87';

export class System_87 implements System {
  public readonly name = 'SpecializedEngineSystem_87';
  public readonly priority = 870;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_87');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_87>(entityId, 'Component_87');
