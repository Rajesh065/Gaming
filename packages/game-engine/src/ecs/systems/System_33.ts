...
export class System_33 implements System {
  public readonly name = 'SpecializedEngineSystem_33';
  public readonly priority = 330;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_33');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_33>(entityId, 'Component_33');
