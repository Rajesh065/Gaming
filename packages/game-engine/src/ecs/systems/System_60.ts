...
import { Component_60 } from '../components/Component_60';

export class System_60 implements System {
  public readonly name = 'SpecializedEngineSystem_60';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_60');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_60>(entityId, 'Component_60');
