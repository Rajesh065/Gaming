...
export class System_20 implements System {
  public readonly name = 'SpecializedEngineSystem_20';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_20');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_20>(entityId, 'Component_20');
