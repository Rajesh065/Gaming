...
export class System_29 implements System {
  public readonly name = 'SpecializedEngineSystem_29';
  public readonly priority = 290;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_29');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_29>(entityId, 'Component_29');
