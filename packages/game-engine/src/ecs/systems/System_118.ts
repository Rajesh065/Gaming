...
import { Component_118 } from '../components/Component_118';

export class System_118 implements System {
  public readonly name = 'SpecializedEngineSystem_118';
  public readonly priority = 1180;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_118');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_118>(entityId, 'Component_118');
