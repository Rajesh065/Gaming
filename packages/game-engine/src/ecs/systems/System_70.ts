...
import { Component_70 } from '../components/Component_70';

export class System_70 implements System {
  public readonly name = 'SpecializedEngineSystem_70';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_70');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_70>(entityId, 'Component_70');
