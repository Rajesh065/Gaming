...
export class System_5 implements System {
  public readonly name = 'SpecializedEngineSystem_5';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_5');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_5>(entityId, 'Component_5');
