...
import { Component_119 } from '../components/Component_119';

export class System_119 implements System {
  public readonly name = 'SpecializedEngineSystem_119';
  public readonly priority = 1190;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_119');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_119>(entityId, 'Component_119');
