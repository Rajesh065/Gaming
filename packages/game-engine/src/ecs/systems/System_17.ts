...
export class System_17 implements System {
  public readonly name = 'SpecializedEngineSystem_17';
  public readonly priority = 170;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_17');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_17>(entityId, 'Component_17');
