...
export class System_26 implements System {
  public readonly name = 'SpecializedEngineSystem_26';
  public readonly priority = 260;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_26');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_26>(entityId, 'Component_26');
