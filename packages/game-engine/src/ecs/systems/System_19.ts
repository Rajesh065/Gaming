...
export class System_19 implements System {
  public readonly name = 'SpecializedEngineSystem_19';
  public readonly priority = 190;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_19');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_19>(entityId, 'Component_19');
