...
export class System_7 implements System {
  public readonly name = 'SpecializedEngineSystem_7';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_7');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_7>(entityId, 'Component_7');
