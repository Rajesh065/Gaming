...
export class System_6 implements System {
  public readonly name = 'SpecializedEngineSystem_6';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_6');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_6>(entityId, 'Component_6');
