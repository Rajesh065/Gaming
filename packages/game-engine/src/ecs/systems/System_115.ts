...
import { Component_115 } from '../components/Component_115';

export class System_115 implements System {
  public readonly name = 'SpecializedEngineSystem_115';
  public readonly priority = 1150;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_115');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_115>(entityId, 'Component_115');
