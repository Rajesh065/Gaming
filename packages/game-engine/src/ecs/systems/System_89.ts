...
import { Component_89 } from '../components/Component_89';

export class System_89 implements System {
  public readonly name = 'SpecializedEngineSystem_89';
  public readonly priority = 890;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_89');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_89>(entityId, 'Component_89');
