...
import { Component_110 } from '../components/Component_110';

export class System_110 implements System {
  public readonly name = 'SpecializedEngineSystem_110';
  public readonly priority = 1100;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_110');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_110>(entityId, 'Component_110');
