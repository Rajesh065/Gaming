...
import { Component_44 } from '../components/Component_44';

export class System_44 implements System {
  public readonly name = 'SpecializedEngineSystem_44';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_44');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_44>(entityId, 'Component_44');
