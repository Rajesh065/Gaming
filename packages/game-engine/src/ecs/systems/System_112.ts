...
import { Component_112 } from '../components/Component_112';

export class System_112 implements System {
  public readonly name = 'SpecializedEngineSystem_112';
  public readonly priority = 1120;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_112');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_112>(entityId, 'Component_112');
