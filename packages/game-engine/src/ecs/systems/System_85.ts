...
import { Component_85 } from '../components/Component_85';

export class System_85 implements System {
  public readonly name = 'SpecializedEngineSystem_85';
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_85');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_85>(entityId, 'Component_85');
