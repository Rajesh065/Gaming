...
import { Component_74 } from '../components/Component_74';

export class System_74 implements System {
  public readonly name = 'SpecializedEngineSystem_74';
  public readonly priority = 740;
  ...
  public update(world: ECSWorld, deltaTime: number): void {
    const entities = world.query('Component_74');
    const startTime = performance.now();

    for (const entityId of entities) {
      const comp = world.getComponent<Component_74>(entityId, 'Component_74');
